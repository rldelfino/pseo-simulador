...
<head>
    ...
    <title>Simule o Financiamento e Amortização | {banco} | Simulador Datalab</title>
...
<body class="text-slate-800 antialiased min-h-screen flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                <a href="index.html" class="flex items-center space-x-2.5">
                    ...
                    <span class="font-bold text-xl tracking-tight text-teal-600">Simulador Datalab</span>
                </a>
    ...
    <script>
        ...
        function atualizarLinksWhatsapp(vImovel, entrada, valorAmortizar, economiaJuros) {{
            ...
            const textoCta = `Olá! Fiz uma simulação de financiamento e amortização no Simulador Datalab:\n\n` +
                `• Banco: ${{bancoNome}}\n` +
...
def gerar_index_home(pasta_saida, links_por_banco):
    ...
    for banco, links in links_por_banco.items():
        ...
        links_html = "".join([f'''
            <li>
                <a href="{item["slug"]}.html" class="group flex items-center justify-between p-3 rounded-lg hover:bg-teal-50 transition-colors border border-transparent hover:border-teal-100">
                    <span class="text-sm font-medium text-slate-600 group-hover:text-teal-700">
                        {item["texto"]}
                    </span>
                    <svg class="w-4 h-4 text-slate-300 group-hover:text-teal-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                </a>
            </li>
        ''' for item in links])
        ...
        <div class="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 border border-slate-100 overflow-hidden">
            ...
            <!-- Lista de Links -->
            <div class="p-4">
                <ul class="space-y-2 h-60 overflow-y-auto pr-2 custom-scrollbar">
                    {links_html}
                </ul>
            </div>
        </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Simulador Datalab | Portal de Financiamento e Amortização Imobiliária</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
</head>
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    <div class="bg-slate-900 py-20 text-center px-4 relative overflow-hidden">
        ...
        <h1 class="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight max-w-4xl mx-auto">Calcule seu financiamento e o impacto das amortizações</h1>
